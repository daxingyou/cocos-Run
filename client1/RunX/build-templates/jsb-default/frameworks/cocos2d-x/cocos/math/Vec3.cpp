...
void Vec3::transformMat4Normal(const Vec3& v, const Mat4 &m)
{
    float ix = v.x, iy = v.y, iz = v.z;
    float rhw = m.m[3] * ix + m.m[7] * iy + m.m[11] * iz;
    rhw = rhw ? 1 / rhw : 1;

    x = (m.m[0] * ix + m.m[4] * iy + m.m[8] * iz) * rhw;
    y = (m.m[1] * ix + m.m[5] * iy + m.m[9] * iz) * rhw;
    z = (m.m[2] * ix + m.m[6] * iy + m.m[10] * iz) * rhw;
}
