 ...
 * @LastEditors: Fly
 * @LastEditTime: 2022-03-22 15:03:43
 * @Description: 请填写简介
 */
import { comm, worldsvr } from "./lib/protocol"
import { AccoutInfo } from "./OperationSvr"
...
type SuccCallback = (res: worldsvr.CheckAccountRes) => void
type FailCallback = (errdesc: string) => void

export default class WorldSvr {
    private _socket: WebSocket = null
    private _timer: number = 0

    private doneSucc(succ: SuccCallback, res: worldsvr.CheckAccountRes) {
        this.clear()
        succ(res)
    }
    ...
    private genCheckAccountReqMsg(accinfo: AccoutInfo): ArrayBuffer {
        let req = worldsvr.CheckAccountReq.create({
        ...
        let request = comm.Request.create({
            Seq: 0,
            CMD: worldsvr.CMD.CHECK_ACCOUNT_REQ,
            Msg: worldsvr.CheckAccountReq.encode(req).finish(),
        })
    ...
    checkAccount(accinfo: AccoutInfo, succ: SuccCallback, fail: FailCallback) {
        this.clear()

        this._socket = new WebSocket(svrConfig.worldsvr)
        this._socket.binaryType = "arraybuffer"
        
        this._timer = setTimeout(() => {
            this.doneFail(fail, predefine.errdescTimeout) 
        }, predefine.timeout)

        this._socket.onopen = (ev: Event) => {
            if (this._socket != ev.target) return
            this._socket.send(this.genCheckAccountReqMsg(accinfo))
        }
        ...
        this._socket.onmessage = (ev: MessageEvent) => {
            ...
            try {
                ...
                let res = worldsvr.CheckAccountRes.decode(response.Msg)
