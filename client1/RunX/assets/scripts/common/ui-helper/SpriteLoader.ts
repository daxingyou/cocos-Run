...
class SpriteLoader {
    ...
    changeSprite(sp: cc.Sprite, url: string, callback?: (err?: any) => void) {
        ...
        if (this._loaded.has(sp)) {
            ...
            if (info.url == url && sp.spriteFrame) {
                callback && callback();
                return;
            }
        }
    ...
    /**
     * @desc 配合其他方法使用，清理已经加载的精灵节点
     * @param sp 精灵
     */
    deleteSprite(sp: cc.Sprite) {
        if (this._loaded.has(sp)) {
            this._loaded.delete(sp);
        }
        sp.spriteFrame = null;
    }
    ...
    /**
     * @【已经禁用】
     * @desc 整体参照 changeSprite方法，加入资源地址检查，避免异步加载资源导致的顺序问题
     * @param {cc.Sprite} sp 需要更换的Sprite
     * @param {string} url 新的Sprite的资源路径
     * @param {() => void} [callback] 完成的回调
     * @returns
     * @memberof AsyncSpriteLoadHelper
     */
    changeSpriteWithCheck(sp: cc.Sprite, url: string, callback?: (err?: any) => void) {
        if (!sp) {
            callback && callback('Can not change sprite for NULL!!!');
            return;
        }
        ...
        resourceManager.load(url, cc.SpriteFrame, CACHE_MODE.NONE, tag)
            .then(info => {
                if (this._loadQueue.has(tag)) {
                    let info = this._loadQueue.get(tag);
                    //加载的不是最新资源
                    if (info.url != url) {
                        return;
                    }
