...
export default class ListItem extends cc.Component {
    ...
    _selectedFlagCache: cc.Node = null;
    ...
    //父容器滚动中
    get listScrolling() {
        return (this.list && this.list.scrolling);
    }
    //依赖的List组件
    public list: List;
    //是否已经注册过事件
        ...
        switch (aniType) {
            case 0: //向上消失
                tween = cc.tween(t.node)
                    .to(.06, { scale: .7 })
                    .by(.2, { y: t.node.height * 2 });
                break;
            case 1: //向右消失
                tween = cc.tween(t.node)
                    .to(.06, { scale: .7 })
                    .by(.2, { x: t.node.width * 2 });
                break;
            case 2: //向下消失
                tween = cc.tween(t.node)
                    .to(.06, { scale: .7 })
    ...
    clearSelectFlag(){
        this._selectedFlagCache = this.selectedFlag;
    ...
    onClickThis() {
        ...
        if (this.list && this.list.scrolling) return;
        // 单选可取消
        if (this.selected && this.list.isToggle){
            this.list.selectedId = -1; 
            return;
        } 
    ...
    sendSelectEvent() {
        this.list.sendSelectChangeEvent();
    }

    inList () {
        if (!this.list || !this.list.scrollView) {
            return false
        }
        return true
    }
    ...
    deInit() {
        if(cc.isValid(this._selectedFlagCache)) {
            this.selectedFlag = this._selectedFlagCache;
            this._selectedFlagCache = null;
        }
    }
}
