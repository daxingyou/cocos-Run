...
/**
 * @【注意！使用必看】
 * 1. 如果组件引用的预制有动态加载或者预加载使用的，则必须不能直接把该预制拉到List组件！否则关闭的时候会释放预制，之后动态加载出问题
 * 2. 如果Item有动态加载资源的，在ItemSpecialComp里面必须填入组件名和补充对应的unuse函数
*/
...
import TempNodePool from './TempNodePool';
...
export default class List extends cc.Component {
    ...
    @property({ type: cc.Enum(TemplateType), tooltip: CC_DEV && '模板类型（Node类型禁用）', })
    set templateType(t: TemplateType) {
        this._templateType = t;
        if(this._templateType != TemplateType.NODE){
            this.tmpNode = null;
        }

        if(this._templateType != TemplateType.PREFAB){
            this.tmpPrefab = null;
        }
    }
    ...
    private _externalPool: TempNodePool<cc.Component> = null;

    get externalPool() {
        return this._externalPool;
    }

    //链接外部节点缓存池，需要在设置数据源之前就调用
    setupExternalPool<T extends cc.Component>(pool: NodePool<T>) {
        if(this.templateType != TemplateType.EXTERNAL_POOL) {
            cc.warn('List the function named \'setupExternalPool\' can be called when templateType is TemplateType.EXTERNAL_POOL');
            return;
        }
        this._externalPool = new TempNodePool(pool);
        let tmpComp = this._externalPool.get();
        if (cc.isValid(this._itemTmp)) {
            cc.warn('[List Component] itemTmp is valid while create custom pool')
        }
        this.setTemplateItem(tmpComp.node);
    }
    ...
    @property({
        tooltip: CC_DEV && '需要初始化对item做初始化和反初始化时候，item绑定的特定组件名，初始化方法reuse，反初始化方法unuse',
    })
    itemSpecialComp: string = '';

    //是否为循环列表
    @property({
        tooltip: CC_DEV && '是否为循环列表',
        visible() {
            let val: boolean = /*this.virtual &&*/ this.slideMode == SlideType.NORMAL || this.slideMode == SlideType.PAGE;
            if (!val)
                this.cyclic = false;
            return val;
        }
    ...
    @property({
        type: cc.Integer,
        tooltip: CC_DEV && '多选最大限制个数',
        visible() { return this.selectedMode == SelectedType.MULT}
    })
    private multipleMaxNum: number = 999;
    //触发选择事件
    @property({
        type: cc.Component.EventHandler,
    ...
    @property({
        ...
        tooltip: CC_DEV && '页面指示器',
        visible() { return true }
    })
    private indicator: cc.PageViewIndicator = null;

    //手动刷新
    @property({
        tooltip: CC_DEV && '支持手动刷新',
        visible() { return true }
    })
    private handInit: boolean = false;
    ...
    set selectedId(val: number) {
        ...
        switch (t.selectedMode) {
            ...
            case SelectedType.MULT: {
                ...
                // 判断是否新选择且达最大个数，是，则返回
                if (t.multSelected.indexOf(val) === -1 && t.multSelected.length >= t.multipleMaxNum) {
                    return;
                }
    ...
    set numItems(val: number) {
        ...
        if (this.indicator) this.indicator._refresh();
        // fly: 重置数据后，点击同个idx item没有响应
        // this._selectedId = -1;
    }
    get numItems() {
    ...
    onDestroy() {
        ...
        if (cc.isValid(t._itemTmp, true))
            t._itemTmp.destroy();
        if (cc.isValid(t.tmpNode, true))
            t.tmpNode.destroy();
    ...
    private _deInitUsedNode() {
        let children = this._getContentChildren();

        //使用外部节点池
        if(this.templateType == TemplateType.EXTERNAL_POOL){
            if (this.content && children.length > 0) {
                for (let n: number = children.length - 1; n >= 0; n--) {
                    let item: any = children[n];
                    let listItem = item.getComponent(ListItem);
                    cc.isValid(listItem) && listItem.deInit();
                    this._pool && this._pool.put(item);
                }
            }
            return;
        }
        ...
        if (this.content && children.length > 0) {
            for (let n: number = children.length - 1; n >= 0; n--) {
                let item: any = children[n];
                if (this.itemSpecialComp && this.itemSpecialComp.length > 0) {
                    let comp = item.getComponent(this.itemSpecialComp);
                    comp && comp.unuse && comp.unuse();
                }
            }
        }

        if (this._pool && cc.isValid(this._pool)) {
            for (let i = this._pool.size() - 1; i >= 0; i--) {
    ...
    _init() {
        ...
        t.templateType != TemplateType.EXTERNAL_POOL && t.setTemplateItem(cc.instantiate(t.templateType == TemplateType.PREFAB ? t.tmpPrefab : t.tmpNode));
        ...
        if(this.templateType == TemplateType.EXTERNAL_POOL){
            ...
            t._pool.clear = () => {
                //与外部池断开连接
                this._externalPool = null;
            }

            t._pool.put = (item: cc.Node) => {
                if(this._externalPool){
                    this._externalPool.put(item.getComponent(this._externalPool.itemType));
                    return;
                }
                cc.warn('List \'externalPool\' is null, please init! ')
            }

            t._pool.get = (...rest: any[]) => {
                if(this._externalPool) {
                    return this._externalPool.get();
                }
                cc.warn('List \'externalPool\' is null, please init! ');
                return null;
            }
    ...
    _deInit(){
        ...
        //使用节点池
        if(this.templateType == TemplateType.EXTERNAL_POOL){
            let isItemTmpValid = cc.isValid(this._itemTmp);
            if(isItemTmpValid) {
                let listItem = this._itemTmp.getComponent(ListItem);
                cc.isValid(listItem) && listItem.deInit();
            }
            isItemTmpValid && this._pool && this._pool.put(this._itemTmp);
            this._itemTmp = null;
            this.externalPool && this.externalPool.clear();
        }
    }
    /**
    ...
    adhere(time?: number) {
        let t: any = this;
        if (!t.checkInited() || this.scrolling)
            return;
    ...
    _createOrUpdateItem(data: any) {
        ...
        if (!item) { //如果不存在
            //使用外部节点池的情况下，canget始终为true
            let canGet: boolean = this._pool.size() > 0;
            item = this._getItem(data.id);

            //外部对象池返回的是cc.Component的实例，需要转换成Node
            if(this.templateType == TemplateType.EXTERNAL_POOL){
                item = item.node;
            }
            ...
            if (!canGet) {
                //新创建的item需要调用reuse
                if (this.itemSpecialComp && this.itemSpecialComp.length > 0) {
                    let specialComp = item.getComponent(this.itemSpecialComp);
                    specialComp && specialComp.reuse && specialComp.reuse(data.id);
                }
            }
            ...
            if (!item.parent || item.parent !== this.content) {
            ...
            item.setSiblingIndex(this._getContentChildren().length - 1);
    ...
    _createOrUpdateItem2(listId: number) {
        ...
        if (!item) { //如果不存在
            ...
            if (this.itemSpecialComp && this.itemSpecialComp.length > 0) {
                let specialComp = item.getComponent(this.itemSpecialComp);
                specialComp && specialComp.reuse && specialComp.reuse(listId);
            }
            this.content.addChild(item);
            listItem = item.getComponent(ListItem);
            item['listItem'] = listItem;
    ...
    _delSingleItem(item: any) {
        ...
        if (this.itemSpecialComp && this.itemSpecialComp.length > 0) {
            let specialComp = item.getComponent(this.itemSpecialComp);
            if (specialComp) {
                specialComp.unuse && specialComp.unuse();
            }
        }
    ...
    scrollTo(listId: number, timeInSecond: number = .5, offset: number = null, overStress: boolean = false) {
        let t = this;
        if (!t.checkInited(false) || (!listId && listId!=0))
            return;
    ...
    //获取当前Index
    getCurIndex() {
        return this.nearestListId;
    }
    //给页面指示器使用
    getPages() {
        return new Array(this.numItems);
    }
    get _curPageIdx() {
        return this.nearestListId<this.numItems ? this.nearestListId : 0;
    }
    //扩展，从子Item强制向列表发送一次选中更新
    sendSelectChangeEvent() {
        let t = this;
        let item = this.getItemByListId(t.selectedId);
        if (t.selectedEvent) {
            let curr = new Date().getTime();
            if (curr - appCfg.globalClick > GLOBAL_CLICK_INTERVAL) {
                appCfg.globalClick = curr
                cc.Component.EventHandler.emitEvents([t.selectedEvent], item, t.selectedId % this._actualNumItems, t._lastSelectedId == null ? null : (t._lastSelectedId % this._actualNumItems));       
            } else {
    ...
    //从对象池中获取新节点
    private _getItem(...rest: any[]) {
        if(this._pool.size() > 0){
            return this._pool.get(...rest);
        }

        return cc.instantiate(this._itemTmp);
    }

    // 获取content子节点数组（排除已放入池子的）
    private _getContentChildren() {
        let children: cc.Node[] = [];
        if(!cc.isValid(this.content)) return children;
        let child: any = null;
        for (let n: number = this.content.childrenCount - 1; n >= 0; n--) {
            child = this.content.children[n];
            
            // 如果已经在TempNodePool中，则跳过
            if (this._externalPool && this._externalPool.find(child.getComponent(this._externalPool.itemType))) {
                continue;
            }
