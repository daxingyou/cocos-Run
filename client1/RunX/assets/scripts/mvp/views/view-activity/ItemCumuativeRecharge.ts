...
@ccclass
export default class ItemCumuativeRecharge extends cc.Component {
    ...
    private _initView() {
        ...
        if(!period) return;
        let cumulativeRechargeData = activityData.cumulativeRechargeData;
        if (cumulativeRechargeData) {
            let rechargeData = activityData.cumulativeRechargeData.ActivityAmountRechargePeriodMap[period] || {};
            ...
            let desc = configUtils.getDialogCfgByDialogId(99000059).DialogText;
    ...
    onClickRewardBtn() {
        ...
        if (period)
            activityOpt.takeCumulativeRechargeReward(period, [this._rechargeCfg.Num]);
    }
