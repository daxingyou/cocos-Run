...
export default class GameLoading extends cc.Component {
    ...
    private _initPosition () {
        if (this.progress) {
            ...
            this.progressActionNode.x = -this.progress.totalLength * 0.5;
        }
    }
    ...
    update(dt: number) {
        ...
        this.progressActionNode.x = this.progress.progress * this.progress.totalLength - this.progress.totalLength * 0.5;
        if (this.progress.progress >= 1) {
