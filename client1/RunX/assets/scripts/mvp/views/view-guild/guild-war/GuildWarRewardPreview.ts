...
import ItemRankRewardPreview from "../../view-pvp/pvp-peakduel/ItemRankRewardPreview";

const {ccclass, property} = cc._decorator;
@ccclass
export default class GuildWarRewardPreview extends ViewBaseComponent {
    ...
    private _prepareData() {
        let rankNoCfg = configManager.getConfigs("guildBattleRank");
        ...
        if (!selfRankinfo || !pvpData.peakDuelRankInfo.PVPPeakDuelIntegralList) {
            this.selfRankNo.string = '当前排名未上榜'
        } else {
            ...
            this.selfRankNo.string = (index >= 0) ? (`当前排名: ${index+1}`) : "未上榜";
        }
    }
