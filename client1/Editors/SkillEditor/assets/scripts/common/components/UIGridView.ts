...
export default class UIGridView extends cc.Component {
    ...
    @property({
        type: cc.Float,
        tooltip: "行间距（水平模式下，会自动修正非法数值）"
    })
    spacingRow = 0;

    @property({
        type: cc.Float,
        tooltip: "列间距（垂直模式下，会自动修正非法数值）"
    })
    spacingCol = 0;
