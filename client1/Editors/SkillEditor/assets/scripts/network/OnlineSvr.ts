import { comm, onlinesvr } from "./lib/protocol"
...
type SuccCallback = (res:onlinesvr.QueryGamesvrsRes)=>void
type FailCallback = (errdesc:string)=>void

export default class OnlineSvr {
    private _socket: WebSocket = null
    private _timer: number = 0

    private doneSucc(succ:SuccCallback, res:onlinesvr.QueryGamesvrsRes) {
        this.clear()
        succ(res)
    }
    ...
    private genQueryGamesvrsReqMsg(): ArrayBuffer {
        let req = onlinesvr.QueryGamesvrsReq.create({
        })

        let request = comm.Request.create({
            Seq: 0,
            CMD: onlinesvr.CMD.QUERY_GAMESVRS_REQ,
            Msg: onlinesvr.QueryGamesvrsReq.encode(req).finish(),
        })
    ...
    queryGamesvrs(succ:SuccCallback, fail:FailCallback) {
        this.clear()

        this._socket = new WebSocket(svrConfig.onlinesvrUrl)
        ...
        this._socket.onopen = (ev:Event)=>{
            ...
            this._socket.send(this.genQueryGamesvrsReqMsg())
        }
        ...
        this._socket.onmessage = (ev:MessageEvent)=>{
            ...
            try {
                ...
                let res = onlinesvr.QueryGamesvrsRes.decode(response.Msg)
