...
import { TARGET_TYPE, ROLE_TYPE, EffectType, SkillType } from "../../../app/AppEnums";
...
class BuffManager {
    ...
    process (buffId: number, user: number): Effect[]  {
        let result: Effect[] = [];
        let realTarget: BTBaseRole[] = [];

        let userRole = dataManager.battleData.getRoleByUid(user);
        let cfg = configUtils.getBuffConfig(buffId);
        realTarget = this._getTargetsFromCfg(userRole, cfg.TargetType);

        if (cfg && cfg.EffectID) {
            result = result.concat(this._getBuffEffect(userRole, cfg, realTarget));
        }
        return result;
    }

    private _getBuffEffect (user: BTBaseRole, buffCfg: any, realTargets?: BTBaseRole[]): Effect[] {
        let effectIdStr: string = buffCfg.EffectID;
        let targetType = buffCfg.TargetType;
